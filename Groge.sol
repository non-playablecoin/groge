contract ERC420 is Ownable {
    ...
    function _nft_burn(address from, uint256 id, uint256 value) internal {
        if (from == address(0)) {
    ...
    function _nft_mint(address to, uint256 id, uint256 value) internal {
        if (to == address(0)) {
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (!isInAllowlist(to)) {
            uint256 tokens_to_mint = ((balanceBeforeReceiver + value) /
                10 ** _decimals) - (balanceBeforeReceiver / 10 ** _decimals);
